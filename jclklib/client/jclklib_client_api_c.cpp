...
bool jcl_c_connect(jcl_c_client_ptr client_ptr) {
    return static_cast<JClkLibClient::JClkLibClientApi*>(client_ptr)->jcl_connect();
}

bool jcl_c_disconnect(jcl_c_client_ptr client_ptr) {
    return static_cast<JClkLibClient::JClkLibClientApi*>(client_ptr)->jcl_disconnect();
}
...
bool jcl_c_subscribe(jcl_c_client_ptr client_ptr,
    ...
    ret = static_cast<JClkLibClient::JClkLibClientApi*>(client_ptr)->jcl_subscribe(newsub, state);
    if (ret == false)
        return ret;
